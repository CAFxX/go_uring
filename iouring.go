...
func (r *iouring) putsv(sv []sche) {
	...
	for si, s := range sv {
		...
		case opRead:
			...
			*(*C.ulong)(unsafe.Pointer(&sqe.anon0[0])) = C.ulong(s.pos) // sqe.off = s.pos
		...
		case opAppend:
			...
			sqe.addr = C.ulonglong(uintptr(unsafe.Pointer(&iovecs[si])))
			sqe.len = 1
			*(*C.__kernel_rwf_t)(unsafe.Pointer(&sqe.anon1[0])) = C.RWF_APPEND // sqe.rw_flags = RWF_APPEND
...
// AppendFile writes the contents of buf appending to file f.
// Returns the number of bytes written, or an error.
...
// WriteFile writes the contents of buf appended to file f.
// Returns the number of bytes written, or an error.
func WriteFile(f *os.File, buf []byte) (int64, error) {
	once.Do(global.Init)
	return global.WriteFile(f, buf)
}

// AppendFile writes the contents of buf in file f starting from offset pos.
// Returns the number of bytes written, or an error.
func AppendFile(f *os.File, buf []byte, pos int64) (int64, error) {
	once.Do(global.Init)
	return global.AppendFile(f, buf, pos)
}
